...
export async function loadPack(packId: string): Promise<Pack> {
  ...
  const response = await fetch(`/packs/${packId}/metadata.json`, NO_STORE_FETCH);
  if (!response.ok) {
  ...
  const normalizedPack: Pack = {
    ...pack,
    puzzles: [...pack.puzzles].sort((a, b) => a.localeCompare(b)),
  };
  
  // Store in cache
  packMetadataCache.set(packId, normalizedPack);
...
export async function loadPuzzle(packId: string, puzzleId: string): Promise<Puzzle> {
  ...
  const response = await fetch(`/packs/${packId}/puzzles/${puzzleId}.json`, NO_STORE_FETCH);
  if (!response.ok) {
...
export async function loadPacksList(): Promise<PackSummary[]> {
  ...
  packsListCache.promise = (async () => {
    ...
    const response = await fetch('/packs/index.json', NO_STORE_FETCH);
    if (!response.ok) {
    ...
    const sortedPacks = sortById(packs);
    
    // Store in cache
    packsListCache.data = sortedPacks;
